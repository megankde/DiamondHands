...
const char * const BITCOIN_CONF_FILENAME = "diamondhands.conf";
const char * const BITCOIN_PID_FILENAME = "diamondhandsd.pid";
...
#else
    const char* pszModule = "diamondhands";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DiamondHands";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DiamondHands";
#else
    // Unix
    return pathRet / ".diamondhands";
#endif
#endif
}
